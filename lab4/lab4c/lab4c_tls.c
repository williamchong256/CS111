...
void connectSSL()
{
    ...
    if (new_context == NULL)
    {
        fprintf(stderr, "Error getting SSL context.\n");
        exit(1);
    }
    ssl_client = SSL_new(new_context);
    if (ssl_client == NULL)
    {
        fprintf(stderr, "Error completing SSL setup.\n");
        exit(1);
    }
    
    //SSL Connection
    if (!SSL_set_fd(ssl_client, sockfd)) //SSL_set_fd() returns 0 if fails
    {
        fprintf(stderr, "Error associating TCP socket with SSL client.\n");
        exit(1);
    }
    
    int ret = SSL_connect(ssl_client);
    
    if (ret < 0) //fatal errror at protocol level or connection error
    {
        fprintf(stderr, "TLS/SSL handshake not successful because fatal error occured at protocol level, or connection failure occured.\n");
        exit(1);
    }
    else if (ret == 0) //handshake rejected but shutdown correctly
    {
        fprintf(stderr, "TLS/SSL handshake not successful, but followed shutdown protocol.\n");
        exit(1);
    }
}
...
void sendID()
{
    ...
    {
        fprintf(stderr, "Error writing with SSL_write() to server socket.\n");
        exit(1);
    }
...
void generateReport(const double tempval)
{
    ...
    if (reportwrite)
    {
        ...
        if (SSL_write(ssl_client, buff, strlen(buff)) < 0)
        {
            fprintf(stderr, "Error writing with SSL_write().\n");
            exit(1);
        }
...
void shutdown_handler()
{
    ...
    if (SSL_write(ssl_client, shutdown, strlen(shutdown)) < 0)
    {
        fprintf(stderr, "Error writing with SSL_write().\n");
        exit(1);
    }
...
//    }
    ...
    while (run_flag)
    {
        ...
        while (difftime(current, begin) < period)
        {
            ...
            {
                num = SSL_read(ssl_client, keyboard_buff, 128);
